...
class User(AbstractUser):
    ...
    salary = MoneyField(decimal_places=2, null=False, blank=False, default=Decimal(0), default_currency='GBP', max_digits=11, validators=[MinMoneyValidator(Decimal(0))])
    ...
    def clean(self):
        if self.is_staff and self.salary is None:
            raise ValidationError('Salary is required for staff members.')
        ...
        if self.salary is not None and self.salary.currency.code != 'GBP':
            raise ValidationError('British pound is only allowed currency for a salary.')
        if self.balance is not None and self.balance.currency.code != 'GBP':
            raise ValidationError('British pound is only allowed currency for a balance.')
...
class Movie(models.Model):
    ...
    image = models.ImageField(upload_to='static/cinema/storage/', null=True, blank=True, editable=True)
    ...
    def get_all_movies(self):
            return Movie.objects.order_by('starts_at')
